...
select CUS_GENDER, count(CUS_ID) as "Total no. of customer" from(
select * from 
	(select `ORDER`.CUS_ID, customer.CUS_GENDER, customer.CUS_Name from customer 
	inner join `ORDER` on `ORDER`.CUS_ID=customer.CUS_ID where `ORDER`.ORD_AMOUNT>=3000) AS t1
    group by t1.CUS_ID) as t2
    group by t2.CUS_GENDER;

select `order`.CUS_ID, product.PRO_Name, supplier_pricing.PRO_ID from product 
	inner join supplier_pricing on product.PRO_ID=supplier_pricing.PRO_ID
    inner join `order` on supplier_pricing.PRICING_ID=`order`.PRICING_ID where CUS_ID=2;
    
select supplier.* from supplier where supplier.SUPP_ID in
(select SUPP_ID from  supplier_pricing group by SUPP_ID having count(SUPP_ID)>1);
   
select category.CAT_NAME, category.CAT_ID, min(t2.Min_Price) as Min_Price from category inner join
(select product.PRO_NAME, product.CAT_ID, t1.* from product inner join  
	(select PRO_ID, min(SUPP_PRICE)as Min_Price from supplier_pricing group by PRO_ID)
	as t1 on product.PRO_ID=t1.PRO_ID) 
    as t2 on category.CAT_ID=t2.CAT_ID group by CAT_ID;

select t2.PRO_ID, PRO_NAME from product inner join	
	(select PRO_ID from supplier_pricing inner join 
	(select PRICING_ID from `order` where ORD_DATE>"2021-10-05") 
    as t1 on supplier_pricing.PRICING_ID=t1.PRICING_ID) 
